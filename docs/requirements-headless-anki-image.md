...
**Target:** Anki via PyPI `aqt==26.9.3` on Python 3.12 (the version headless-anki already ships
...
Layered on `x11-vnc` (which already brings `python:3.12-slim`, `aqt==26.9.3`, Qt6/xcb libs,
`Xvfb`, `openbox`, `x11vnc`, `curl`, `jq`, `unzip`, `mpv`, `lame`, locale).

| Component | Source / pin | Why | Notes |
|---|---|---|---|
| **Anki** | `aqt==26.9.3` (inherited from `base`) | The app | Pin matches ci-buddy target; do not bump silently |
...
- [ ] Anki `aqt==26.9.3` / py3.12; `ci_buddy` + AnkiMCP add-ons baked into an **image stash**
